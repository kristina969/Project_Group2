...
*we dont have earnings forecasts forecast in june every year --> 
so we take the last forecast that closest to 30.06) ;
...
* we use the beta coefficients as of 30.06 for the respective HouYear;
 proc sql;
	...
	having a.houdate = max(a.houdate);   *brauchen wir das?;
quit;
...
*Mild outlier definition: observations that fall outside of [Q1-1.5*IQR, Q3+1.5*IQR];
...
data homework.model_variables_2;
	...
	EV_Sales = (csho*prcc_f+lt)/sale;label EV_Sales = "Enterprice value sales multipe with outliers";
	Risk_m = Beta_m; label Risk_m = "Risk";
run;


*Keep only observations for which EV_Sales-valriable is not missing;
